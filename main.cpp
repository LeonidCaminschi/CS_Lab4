...
int main() {
    ...
    for(int i=0; i<16; i++) {
        ...
        if (first_28_bits & 0x10000000) {
            first_28_bits = first_28_bits & 0x0FFFFFFF;
            first_28_bits = first_28_bits + 1;
        }

        if (last_28_bits & 0x10000000) {
            last_28_bits = last_28_bits & 0x0FFFFFFF;
            last_28_bits = last_28_bits + 1;
        }

        uint64_t intermediary = (first_28_bits << 28) + last_28_bits;
       cout << hex << intermediary << endl;
        uint64_t reorderedInt = 0;
